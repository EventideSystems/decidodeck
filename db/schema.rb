...
ActiveRecord::Schema[8.0].define(version: 2025_06_18_020451) do
  ...
  SQL
  create_function :checklist_item_at_time, sql_definition: <<-'SQL'
      CREATE OR REPLACE FUNCTION public.checklist_item_at_time(checklist_item_id integer, at timestamp without time zone)
       RETURNS boolean
       LANGUAGE plpgsql
      AS $function$
      ...
      $function$
  SQL
  create_function :checklist_item_status_at_time, sql_definition: <<-'SQL'
      CREATE OR REPLACE FUNCTION public.checklist_item_status_at_time(the_checklist_item_id integer, at timestamp without time zone)
